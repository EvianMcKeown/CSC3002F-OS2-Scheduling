...
public class Patron extends Thread {
	...
	AtomicLong responseTime = new AtomicLong(-1);
	private Random random = new Random();// for variation in Patron behavior
	...
	public void run() {
		try {
			...
			//responseTime - time taken to receive first drink
			...
			for (int i = 0; i < lengthOfOrder; i++) {
				...
				synchronized (responseTime) {
					if ((responseTime.get() > (startTime - System.currentTimeMillis())) || (responseTime.get() == -1)) {
						responseTime.set(System.currentTimeMillis() - startTime - drinksOrder[i].getExecutionTime());
					}
				}
			}
			...
			long averageWaitingTime = totalWaitingTime / drinksOrder.length;
			long averageTime = totalTime / drinksOrder.length;
			
			writeToFile(String.format("%d,%d,%d,%d\n", ID, responseTime.get(), averageTime, averageWaitingTime));
